...
export function DescribeScreens() {
  // Example array of screen designs, as provided
    ...
    },
    {
      ...
      subFiles: [
        ...
        },
        {
          ...
          orderIndex: 4,
          accessUrl:
            "https://d3sqmis56xqhp3.cloudfront.net/Organizations/Organization-579343a4-9e6d-4039-b26d-aff809d6cff0/Project-da96dd3a-c3b1-45cd-9112-514d1aa9a2d5/Epic-b096b57d-2cac-4f1e-8e2f-45b9ef4c42ac/designFiles/Frame 6-2025-03-18T10:30:10.400Z.png",
          parentFileId: "f1dbba6e-0858-4d01-a54d-d27a2ad14c1a",
          designFlowDoc: "",
          epicId: "",
        },
  ...
  const moveItem = (
    ...
    // Recurse into subFiles if it is a subfile
    const moveFile = (file: DesignFileDto) => {
      if (file.subFiles) {
        const subFileIndex = file.subFiles.findIndex(
          (subfile) => subfile.id === draggedItem.id
        );
        if (subFileIndex > -1) {
          const temp = file.subFiles[subFileIndex];
          file.subFiles[subFileIndex] = draggedItem;
          file.subFiles[droppedOnItem.orderIndex] = temp;
          return true;
        }
      }
      return false;
  ...
  function DraggableDesignFileItem(props: {
    ...
    const autoScroll = (event: MouseEvent) => {
      ...
      const scrollSpeed = 10; // How fast the container should scroll
      const threshold = 50; // Distance from the edge to start scrolling
      ...
      if (event.clientY - rect.top < threshold) {
        container.scrollTop = Math.max(scrollTop - scrollSpeed, 0); // Scroll up
    ...
    // Attach the auto-scroll event listener on drag start and remove on drag end
    useEffect(() => {
      if (isDragging) {
